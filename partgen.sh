...
d-i preseed/late_command string vgcreate --yes xenvg /dev/$lvmvol && lvcreate --yes -L 20G -n system-stuff xenvg && mkfs.ext4 /dev/xenvg/system-stuff && echo "/dev/xenvg/system-stuff /stuff ext4 errors=remount-ro 0 0" >>/target/etc/fstab && mkdir -p /target/stuff
EOF
  fi
  ;;
  *)
